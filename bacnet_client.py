...
class BACnetClient(BIPSimpleApplication):
    def __init__(self, gui_update_callback, *args):
        if _debug: BACnetClient._debug("__init__%r", args)
        BIPSimpleApplication.__init__(self, *args)
        self.gui_update_callback = gui_update_callback

    def do_IAmRequest(self, apdu):
        if _debug: BACnetClient._debug("do_IAmRequest %r", apdu)
        ...
        try:
            ...
            print(f"Discovered device: ID={device_info['device_id']}, "
    ...
    def process_read_response(self, iocb):
        if _debug: BACnetClient._debug("do_ReadPropertyACK %r", iocb)
    ...
    def send_whois(self):
            ...
            if _debug: BACnetClient._debug("Send WhoIs request")
    ...
    def read_device_objects(self, device_id):
        if _debug: BACnetClient._debug("read_device_objeccts %r", device_id)
